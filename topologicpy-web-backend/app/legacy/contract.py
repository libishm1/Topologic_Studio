"""Legacy TopologicPy JSON-contract endpoints, moved verbatim from Classic.

This is the ``/upload-topology`` and ``/upload-ifc`` workflow that predates the
IFC egress pipeline. It is carried across unchanged on purpose: it is not on
the performance-critical path, it is the part of the app most entangled with
TopologicPy's own JSON contract, and rewriting it at the same time as the
viewer would have made regressions impossible to attribute.

The only edits versus Classic are the removal of the hard-coded
``sys.path.append`` to a developer's machine, and moving the FastAPI routes
onto an ``APIRouter``.
"""
...
def convert_ifc_to_contract(file_path: str, include_path: bool = False, tilt_min: float = 0.3, max_z_span: float = 1.0, min_floor_area: float = 9.0):
    ...
    # Classic assigned a module-global LAST_GRAPHS here, so two tabs clobbered
    # each other. Both graphs now go into the shared store under their mode.
    ...
    register_legacy_graph("wire", vertex_coords, wire_adjacency)
    register_legacy_graph("cell", cell_coords, cell_adjacency, bboxes=cell_nodes)
